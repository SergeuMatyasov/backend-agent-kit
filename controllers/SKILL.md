...
6. Для публичного leaf-контроллера обязательно задавать явный `[Route(...)]`.
...
Дополнительное правило для block-based controller tree:

- `*ControllerBase`, который используется только для общих API-конвенций (`[ApiController]`,
	`[Produces]` и т.д.), может не иметь собственного `[Route(...)]`.
- Если leaf-контроллер объявляет собственный `[Route(...)]`, он должен задавать полный публичный путь
	целиком, например `api/reviews/ozon`.
- Нельзя рассчитывать на неявную композицию `[Route(...)]` между base controller и leaf controller,
	если конечный path неочевиден при чтении кода и не проверен в OpenAPI.
...
Дополнительное правило для block-based маршрутов:

- В блоке с `*ControllerBase` итоговый публичный path должен определяться однозначно в одном месте.
- Если потомки имеют собственные `[Route(...)]`, использовать полные пути, а не короткие сегменты вроде
	`ozon`, `wildberries`, `images` в надежде на base-prefix.
- Любая route-композиция через inheritance должна быть явно проверена на уникальность `method/path`
	в Swagger/OpenAPI.
...
25. Неявная композиция `[Route(...)]` между base controller и leaf controller, из-за которой итоговый
	public path неочевиден или конфликтует в Swagger/OpenAPI.
...
5. Проверь, что route оформлен ресурсно, однозначно определяется в одном месте и `id` берется из route.
6. Если контроллер находится в block-based tree, проверь, что leaf-контроллер либо задает полный
	публичный path целиком, либо использует один явный и проверенный inheritance-шаблон.
7. Проверь, что request DTO не дублирует route `id` без необходимости.
8. Проверь, что все HTTP DTO имеют явный `JsonPropertyName` на каждом сериализуемом свойстве и строгий `camelCase`.
9. Проверь, что request DTO и response DTO разделены, лежат в отдельных файлах и не используют общий `Contracts.cs`.
10. Проверь, что request DTO маппится в `command/query`, а response DTO маппится из business/application результата.
11. Проверь, что у каждого action-метода есть отдельная папка сценария с файлами `<Scenario>Request.cs` и `<Scenario>Response.cs`.
12. Проверь, что action не содержит бизнес-логики.
13. Проверь, что `CancellationToken` принимается и пробрасывается вниз.
14. Проверь, что все response status codes указаны через `ProducesResponseType`.
15. Проверь, что все `200 OK` и `201 Created` с телом имеют typed response.
16. Проверь, что ошибки возвращаются в едином формате.
17. Проверь, что методы расположены в логическом порядке.
18. Проверь, что XML-документация соответствует endpoint.
19. Проверь, что OpenAPI и тесты должны быть обновлены.
...
- [ ] Route однозначно определяется в одном месте и не собирается неявно из конфликтующих `[Route(...)]`
	base controller и leaf controller.
