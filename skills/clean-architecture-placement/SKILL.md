...
Не переносить код в Shared только потому, что он потенциально может переиспользоваться.
Для Shared обычно ожидаются стабильная ответственность и минимум два конкретных потребителя.
Исключение допустимо для маленьких, чистых, очевидно cross-cutting primitives без явного feature-owner,
если их общая семантика ясна уже в момент создания.
...
11. `Shared` допустимо использовать только для кода без одного явного feature-owner или для маленьких,
  стабильных и очевидно общих cross-cutting primitives.
12. Для переноса в `Shared` обычно нужно назвать минимум два конкретных потребителя; исключение допустимо
  только для маленьких, чистых и очевидно общих primitives без feature-owner.
...
16. Перед созданием нового helper, service, utility или abstraction сначала проверить, нет ли уже эквивалентной реализации в owning feature, shared-коде или соседнем слое.
17. Не выносить feature-specific код в `Shared` и не вводить generic-типы вроде `IService`, `Helper`, `Utils`
  только из-за потенциальной переиспользуемости.
18. Выносить код в переиспользуемый компонент, если reuse реален, уже начинает повторяться или явно ожидаем
  в ближайшем шаге, а ownership и слой названы явно.
19. Если одинаковая логика начинает дублироваться в нескольких местах, считать это триггером для извлечения
  в логичное owning place, а не для дальнейшего копирования.
...
## Правила переиспользования и извлечения
1. Сначала проверяй, нет ли уже существующей реализации с тем же смыслом, прежде чем создавать новую.
2. Если логика похожа, но отличается по смыслу, инвариантам или owner-у, не склеивай ее в искусственную общую абстракцию.
3. Чистую бизнес-логику, реально переиспользуемую внутри домена, выноси в `Domain` как domain service, policy, specification или другой точно названный тип.
4. Если use case нужен переиспользуемый технический capability или внешний вызов, interface этого порта размещай в `Application`, а реализацию - в `Infrastructure`, `EFCore` или `Host`.
5. `Shared` использовать только для стабильного cross-cutting кода, а не как место для кода "на всякий случай".
6. Обычно для `Shared` ожидаются минимум два явных потребителя, но исключение допустимо для маленьких,
  чистых и очевидно общих primitives без feature-owner.
7. Не держи локально код только ради формального ожидания второго использования, если это уже очевидный
  stable cross-cutting primitive.
8. Если одинаковая логика начинает повторяться в нескольких местах, считай это триггером для извлечения
  в owning layer вместо дальнейшего дублирования.
9. Не вводи generic-названия вроде `IService`, `Helper`, `Utils`, если можно назвать более точную доменную
  или техническую ответственность.
10. При извлечении переиспользуемого кода сначала назови owner-а поведения, а потом выбирай слой и папку.
...
7. Вынос feature-specific логики в `Shared` или generic `IService` только потому, что она "может пригодиться".
Почему плохо: появляется premature abstraction, теряется owner и растет связность.
Как правильно: сначала подтвердить реальный reuse, назвать ответственность и разместить код в owning layer.

8. Дублировать уже существующий helper, policy, mapper или service с тем же смыслом в соседней feature.
Почему плохо: поведение начинает расходиться, а поддержка дублируется.
Как правильно: переиспользовать существующую реализацию, если semantics совпадает, либо оставить локальную реализацию, если смысл разный.

9. Держать очевидный cross-cutting primitive локально только ради формального ожидания второго потребителя.
Почему плохо: это искусственно задерживает нормальное извлечение и подталкивает к будущему дублированию.
Как правильно: если primitive уже очевидно общий, маленький и без feature-owner, допускается вынести его сразу в правильное shared place.
...
5. Проверь, нет ли уже существующей реализации или abstraction с тем же смыслом и подходящим owner-ом.
6. Если одинаковая логика уже начинает повторяться, рассмотри извлечение в owning layer вместо нового дублирования.
7. Для use case выбери сценарий и создай отдельную папку `<CaseName>` внутри `UseCases/<Feature>/`.
8. Проверь, не нужен ли новый port interface в `Application` вместо прямой зависимости на infrastructure implementation.
9. Перенеси рядом все зависимые части vertical slice: request, handler, validator, dto/result, exception, tests.
10. Обнови namespace, DI registration, imports контроллеров и tests.
11. Проверь направление зависимостей: наружные слои могут зависеть от внутренних, но не наоборот.
